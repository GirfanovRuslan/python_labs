...
def validate_input_extension(file_path: str, expected_ext: str) -> None:
    ...
    if ext != expected_ext:
        raise ValueError(
            f"Ошибка: входной файл {file_path} должен иметь расширение {expected_ext}"
        )
...
def validate_output_extension(file_path: str, expected_ext: str) -> None:
    ...
    if ext != expected_ext:
        raise ValueError(
            f"Ошибка: выходной файл {file_path} должен иметь расширение {expected_ext}"
        )
    ...
    if len(data) > 1:
        ...
        for i, item in enumerate(data[1:], 1):
            if set(item.keys()) != first_keys:
                raise ValueError(
                    f"Элемент {i} имеет несовпадающие ключи с первым элементом"
                )
...
def json_to_csv(json_file: str, csv_file: str, encoding: str = "utf-8") -> None:
    ...
    try:
        ...
        raise ValueError(
            f"Неверная кодировка файла {json_file}. Попробуйте другую кодировку."
        )
        ...
        with open(csv_file, "r", encoding=encoding) as f:
            ...
            for i, row in enumerate(reader, 1):
                if set(row.keys()) != expected_fields:
                    raise ValueError(
                        f"Строка {i} имеет несоответствующее количество полей"
                    )
                data.append(row)

    except UnicodeDecodeError:
        raise ValueError(
            f"Неверная кодировка файла {csv_file}. Попробуйте другую кодировку."
        )
