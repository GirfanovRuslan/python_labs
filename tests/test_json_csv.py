...
class TestJsonToCsv:
    ...
    def test_file_not_found(self):
        ...
        with pytest.raises(FileNotFoundError):  # ОЖИДАЕМ, что функция выбросит эту ошибку
            json_to_csv("nonexistent.json", "output.csv")  # Вызываем с несуществующим файлом
...
class TestCsvToJson:
    def test_basic_conversion(self, tmp_path):
        ...
        assert data[0] == {"name": "Alice", "age": "22", "city": "Moscow"}  # Первая запись верна
        assert data[1] == {"name": "Bob", "age": "25", "city": "SPb"}      # Вторая запись верна
